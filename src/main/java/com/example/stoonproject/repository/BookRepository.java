...
public interface BookRepository extends JpaRepository<Book, Long> {
    ...
    @Query(value = "SELECT * FROM book WHERE REPLACE(author, ' ', '') LIKE REPLACE(CONCAT('%', :query, '%'), ' ', '')", nativeQuery = true)
    ArrayList<Book> findByQuery_author(@Param("query") String query);
    @Query(value = "SELECT * FROM book WHERE REPLACE(author, ' ', '') LIKE REPLACE(CONCAT('%', :query, '%'), ' ', '')" +
